...
def get_args_parser():
    ...
    parser.add_argument("--video_path", default="", help="path where of video")
    parser.add_argument(
...
def main(args, debug=False):
    ...
    cap = cv2.VideoCapture("vis/demo.mp4")
    ret, frame = cap.read()
    print(frame.shape)
    ...
    out = cv2.VideoWriter("logs/demo.avi", fourcc, 30, (1280, 1280))
        ...
        with torch.no_grad():
            ...
            for p in points:
                img_to_draw = cv2.circle(
                    img_raw, (int(p[0]), int(p[1])), size, (0, 0, 255), -1
                )
            res = np.vstack((ori_img, img_to_draw))

            cv2.putText(
                res,
                "Count:" + str(predict_cnt),
                (30, 30),
